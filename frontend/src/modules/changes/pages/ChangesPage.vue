...
import { fetchChangelog as loadData } from '../api'

const title = '变更中心'
const description = 'ChangeRecord 与影响分析'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:changes`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'changes', sample: items.value[0] || null }
  } catch (err) {
