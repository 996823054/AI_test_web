...
import { fetchApis as loadData } from '../api'

const title = '接口中心'
const description = '接口事实源、环境变量与调试入口'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:apis`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'apis', sample: items.value[0] || null }
  } catch (err) {
