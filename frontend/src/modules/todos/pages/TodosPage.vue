...
import { fetchTodos as loadData } from '../api'

const title = '待办中心'
const description = '待办注册与核销闭环'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:todos`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'todos', sample: items.value[0] || null }
  } catch (err) {
