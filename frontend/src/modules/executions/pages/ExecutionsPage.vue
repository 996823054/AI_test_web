...
import { fetchBatches as loadData } from '../api'

const title = '执行中心'
const description = '任务创建、日志、取消与重试'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:executions`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'executions', sample: items.value[0] || null }
  } catch (err) {
