...
import { fetchPhoenixEvaluators as loadData } from '../api'

const title = 'AI 能力中心'
const description = 'Skill / Prompt / Evaluator 工作台'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:ai`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'ai', sample: items.value[0] || null }
  } catch (err) {
