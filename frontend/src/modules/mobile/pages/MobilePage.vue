...
import { fetchMobileStatus as loadData } from '../api'

const title = '移动端执行中心'
const description = '设备、App 与移动端执行（正式域骨架）'
...
onMounted(async () => {
  try {
    ...
    logs.value = ['module:loaded', `module:mobile`]
    steps.value = [{ name: '加载模块数据', status: 'passed' }]
    evidence.value = { module: 'mobile', sample: items.value[0] || null }
  } catch (err) {
